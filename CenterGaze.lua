--[[
	...
	Add the support for configuring the position and size of tutorial containers 
...
local function apply_patch(id)
	...
	for line in ofh:lines() do
		...
		-- Poor man workaround to fix the CommandMenu.lua previously patched by version < 0.03
		...
		if fn then
			...
			if not done and string.find(line, patch[id].sw) then
				table.insert(lines, patch[id].nl)
				done = true
			else
				table.insert(lines, line)
			end
...
local function center_gaze_hook()
	...
	-- Get rid of previous hook version 
	remove_old_version()

    -- Create the workdir if not exists
    if not dir_exists(hdir) then
        lfs.mkdir(hdir)
    end
    ...
    if not file_exists(cfgf) then
		...
		cfgh:write("defaultCommandMenu = true\n")
		cfgh:write("commandMenuOffset  = 0\n")
		cfgh:write("\n")
		cfgh:write("-- Radio Messages --\n")
		cfgh:write("defaultRadioMessages = true\n")
    ...
    if f then
        ...
        log.error("[CenterGaze] Error: " .. err)
        return
    end

    dlog("CenterGaze v" .. tostring(version) .. " loaded")
    dlog(" - defaultCommandMenu   : " .. tostring(defaultCommandMenu))
    dlog(" - commandMenuOffset    : " .. tostring(commandMenuOffset))
    dlog(" - defaultRadioMessages : " .. tostring(defaultRadioMessages))
    dlog(" - radioMessagesOffset  : " .. tostring(radioMessagesOffset))
    dlog(" - radioMessagesWidth   : " .. tostring(radioMessagesWidth))

    rmo = radioMessagesOffset
    rmw = radioMessagesWidth
    cmo = -1 * commandMenuOffset
    ...
    patch = {
        ...
        msg = {
			df = defaultRadioMessages ,
            sw = "^[ \t]+autoScrollTextRadio:setBounds%(",
            nl = "\tautoScrollTextRadio:setBounds( (main_w/2) + " .. rmo .. ", 40, " .. rmw .. ", main_h - 40)"
        }
    }
		
	if file_changed("cfg") then 
		dlog( "Config file has changes to apply" )
		for id, _ in pairs(patch) do
			local of, bf, tf = files(id)
			if file_exists(bf) then 
				undo_patch(id)
			end 
			if not patch[id].df then 
				apply_patch(id)
			else 
				dlog( "Patching of " .. id .. " file disabled by configuration" )
			end 
		end
		store_modification_time("cfg")
	else 
		dlog( "Config file has no changes to apply" )
		for id, _ in pairs(patch) do
			local of, bf, tf = files(id)
			if file_changed(id) then  
				dlog( "Original " .. id .. " file changed" )
				if not patch[id].df then 
					apply_patch(id)
				else 
					dlog( "Patching of " .. id .. " file disabled by configuration" )
...
if not status then
    log.error("[CenterGaze] Error: " .. tostring(err))
end
